...
func (Jwt) Handler(proxyRoute gobis.ProxyRoute, params interface{}, handler http.Handler) (http.Handler, error) {
	...
	if options.Alg == "" {
		return handler, fmt.Errorf("algorithm can't be empty.")
	}
	if options.Secret == "" {
		return handler, fmt.Errorf("secret can't be empty.")
	}
	...
	jwtMiddleware := jwtmiddleware.New(jwtmiddleware.Options{
		ValidationKeyGetter: func(token *jwt.Token) (interface{}, error) {
			return checkTokenfunc(token, options, signingMethod)
		},
...
func checkTokenfunc(token *jwt.Token, options *JwtOptions, signingMethod jwt.SigningMethod) (interface{}, error) {
